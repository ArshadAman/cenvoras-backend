...
def customer_balance_reconciliation(request):
    ...
    tenant = getattr(request.user, 'active_tenant', request.user)
    selected_customer_id = request.query_params.get('customer') or 'all'
    cache_key = tenant_cache_key('billing', tenant.id, 'customer-reconciliation', selected_customer_id)
...
def item_wise_pl_report(request):
    ...
    cache_key = tenant_cache_key(
        ...
        tenant.id,
        'item-wise-pl',
        request.query_params.get('from', 'all'),
        request.query_params.get('to', 'all'),
    )
